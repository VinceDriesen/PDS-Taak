    ...
    : m_numParticles(numParticles)
{
    ...
    cudaOccupancyMaxPotentialBlockSize(&minGrid, &blockSize_density, computeDensityKernel, 0, m_numParticles);
    gridSize_density = (m_numParticles + blockSize_density - 1) / blockSize_density;

    cudaOccupancyMaxPotentialBlockSize(&minGrid, &blockSize_pressure, computePressureKernel, 0, m_numParticles);
    gridSize_pressure = (m_numParticles + blockSize_pressure - 1) / blockSize_pressure;

    cudaOccupancyMaxPotentialBlockSize(&minGrid, &blockSize_force, computeForcesKernel, 0, m_numParticles);
    gridSize_force = (m_numParticles + blockSize_force - 1) / blockSize_force;

    cudaOccupancyMaxPotentialBlockSize(&minGrid, &blockSize_integrate, integrateKernel, 0, m_numParticles);
    gridSize_integrate = (m_numParticles + blockSize_integrate - 1) / blockSize_integrate;
}
